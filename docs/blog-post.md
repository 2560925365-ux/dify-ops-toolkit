...
Dify 是一个优秀的 LLM 应用开发平台，但在实际生产使用中，我们发现了一个问题：**工作流部署缺乏面向开发者的自动化 API**。

## 痛点

当需要批量部署工作流、创建 API Token 或配置环境变量时：

- Dify 官方文档的 Service API 只能**调用**已部署的应用，不能**部署**应用
- Console API 存在 `/console/api/apps/import` 端点，但未公开文档化，且需要 Console 认证
- 社区多次请求 DevOps 友好的部署 API（[GitHub Discussion #9007](https://github.com/langgenius/dify/discussions/9007)）
...
Dify 的所有配置存储在 PostgreSQL 中。相比未文档化的 Console API，数据库直连有以下优势：
...
**Dify Ops Toolkit** 选择数据库直连，提供更可靠的自动化运维能力。
